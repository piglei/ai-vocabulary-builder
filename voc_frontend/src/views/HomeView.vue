...
const tokenizedText = computed(() => {
	...
	let items = tokens.map(token => {
		let item = {"token": token, "addedToVoc": false, "existingObj": null, "isMastered": false, }
        if (token.type !== 'word') {
		    return item
        }
...
.inputted-text-card span.existing-word {
    margin: 0 6px;
    font-style: italic;
}

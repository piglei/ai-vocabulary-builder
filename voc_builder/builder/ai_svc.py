...
async def get_translation(model, text: str) -> AsyncGenerator[str, None]:
    ...
    try:
        async for translated_text in query_translation(model, text):
...
async def query_translation(model, text: str) -> AsyncGenerator[str, None]:
...
async def get_rare_word(model, text: str, known_words: Set[str]) -> WordChoice:
...
async def get_word_manually(model, text: str, word: str) -> WordChoice:
